...
def generate_step1(target_text,target_data,target):
    ...
    label_ = ['walk', 'sit', 'stairsdown', 'stairsup']
...
def generate_step3(target_text, target_data):
    data = []
    bb = target_text[target_text["label"].isin(['walk', 'sit', 'stairsdown', 'stairsup'])]
    label_ = ['walk', 'sit', 'stairsdown', 'stairsup']
...
def generate_label_cross(motion_text):
    label_ = ['walk', 'sit', 'stairsdown', 'stairsup']
    bb = []
    for j in range(4):
        a = motion_text[motion_text["label"] == label_[j]]
        b = a.sample(n=1)
        bb.append(b["label"].iloc[0])
    return bb


def generate_label_new(target_text, target):
    if target in ["hhar", "motion", "uci", "shoaib"]:
        label_ = ['walk', 'sit', 'stairsdown', 'stairsup']
    bb = []
    for j in range(len(label_)):
        a = target_text[target_text["label"] == label_[j]]
        b = a.sample(n=1)
        bb.append(b["label"].iloc[0])
    return bb
...
def load_dataset(csv_path, label_replace=True):
    ...
    if label_replace:
        df.loc[df["label"] == 'stand', 'label'] = "sit"
