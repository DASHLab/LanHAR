...
def clip_loss_multipos(
    ...
    logits_ab = (z_a @ z_b.t()) * scale      # (B,B) on dev
    logits_ba = (z_b @ z_a.t()) * scale      # (B,B) on dev

    same = labels.unsqueeze(1).eq(labels.unsqueeze(0))  # (B,B) bool on dev
    target_ab = same.float()                             # (B,B)
    target_ba = same.t().float()                         # (B,B)
